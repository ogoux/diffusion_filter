...
def diffusion_sqrt(input_grid, D, M, water_ratio = None, boundary_mask = False,
                       out_of_domain_mask = False, surface_model = 'ellipsoid',
                       boundary_inflation_factor = 1, boundary_inflation_range = 3):
    """
    Square root of the function "diffusion_smoother" if M is chosen even. This 
    operator can be used to generate spatially correlated random samples by 
    filtering white Gaussian noise. The correlation function of the filtered
    sample is then the correlation function of diffusion.
   ...
   Parameters
    ...
    M : int
        ...
        to the smoothing kernel.
    water_ratio : array, optional
        Path to a NetCDF file with a variable ocean_surface_ratio containing the 
        percentage of water surface in each pixel
    boundary_mask : 2D array, optional
        Array of booleans of dimension (nlat,nlon) separating the grid in two domains.
        Neumann conditions are used to restrict fluxes between the two domains 
        (typically if boundary mask is a land mask, there will be no flux between 
    ...
    # The square root operator is only defined for M even
    assert M%2 == 0
    ...
    if (water_ratio is not None) and np.any(boundary_mask)==False:
        boundary_mask =  water_ratio < 1E-16
    # =============================================================================
...
def diffusion_inverse(input_grid, D, M, water_ratio = None, boundary_mask = False,
                       out_of_domain_mask = False, surface_model = 'ellipsoid',
                       boundary_inflation_factor = 1, boundary_inflation_range = 3):
    """
    Inverse of the operator "diffusion_smoother". Note that the operator can 
    become very ill-conditioned for large M, especially around the poles (and to a lesser 
    extent around boundaries), so the inverse should be used with caution. If you
    need the to be able to use the inverse and the result is instable, consider
    lowering M or masking the poles and high laittudes.
